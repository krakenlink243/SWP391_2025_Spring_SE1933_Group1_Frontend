  ...
  return (
    <div className="transaction-steam-bg">
      <div className="transaction-main-steam">
        <h2 className="transaction-title-steam">{t(`'s Purchase History`, {userName: username})}</h2>
        <div className="transaction-list-steam">
          {loading ? (
            <div className="transaction-loading">{t('Loading...')}</div>
          ) : transactions.length === 0 ? (
            <div className="transaction-empty-steam">{t('No transactions found.')}</div>
          ) : (
                  ...
                  {transactions.map((tx, idx) => {
                    ...
                    return (
                      <tr key={idx} className="transaction-row-steam">
                        ...
                        <td className="transaction-td-item">{tx.gameName || tx.item || t('Unknown Item')}</td>
                        <td className="transaction-td-type">{t('Purchase Game')}</td>
                        <td className="transaction-td-amount">{amount === 0 ? t('Free') : `$${amount.toFixed(2)}`}</td>
                        <td className="transaction-td-changes">{amount === 0 ? '' : `- $${amount.toFixed(2)}`}</td>
                        <td className="transaction-td-detail" onClick={() => handleDetailClick(tx.transactionId)}>{t('Detail')}</td>
                      </tr>
                    );
                  })}
                </tbody>
